...
# How to use
The application is executed through the use of container (Docker). The input data was made by text file located on `input/input.txt` with transactions separated line-by-line.
...
## Testing
Enter data via __input.txt__ and run:

```shell
docker-compose up --build
```

The application output is generated in `output/results.txt` and too in container stdout. To access container stdout, run:
